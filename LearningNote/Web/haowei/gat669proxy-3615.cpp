...
#define TEMP_FILE_NAME  "gat669_playbyfile_%d_%s.dat"
#define DATA_IN_BYTE 1024*1024
...
{//gat669_mobotix.dll
    ...
    m_pFileWrite    = NULL;
    m_pFileRead     = NULL;
    m_bReadThreadFinish = TRUE;
    m_byDecoderMode = em_stream_realtime;
...
#endif
	BOOL32 bRet = FALSE;
    if (em_stream_file == m_byDecoderMode)
    {
        StartPlayByFile();
    }
...
BOOL32  CGat669Proxy::SetStreamMode(int nmode)
{
    ...
    bRet = m_cGat669.GAT669_SetStreamOpenMode(m_nPort,nmode);
    if (TRUE == bRet)
    {
        m_byDecoderMode = nmode;
    }
}
...
BOOL32  CGat669Proxy::InputStreamData(void *pData, int dwDataSize,uint32_t dwFrameID)
{
    ...
    BOOL bPlayByFile = IsPlayByFile();
    if (bPlayByFile)
    {
        Input2File(pData,dwDataSize);
    }
    else
    {
        bRet = m_cGat669.GAT669_InputVideoData(m_nPort,(u8*)pData,dwDataSize);
    }
...
BOOL CGat669Proxy::Input2File( void *pData, int dwDataSize )
{
    if ( m_pFileWrite != NULL && !m_bReadThreadFinish)
    {
        fwrite(&dwDataSize, 1, sizeof(dwDataSize), m_pFileWrite);
        fwrite(pData, 1, dwDataSize, m_pFileWrite);
        return TRUE;
    }
    else
    {
        UNILOGE("CGat669Proxy.Input2File m_pFileWrite:%d,m_bReadThreadFinish:%d\n", m_pFileWrite,m_bReadThreadFinish);
        return FALSE;
    }
}

BOOL32 CGat669Proxy::StopPlayByFile()
{
...
BOOL CGat669Proxy::IsPlayByFile( void )
{
    return m_hReadThread != NULL;
}

BOOL32 CGat669Proxy::StartPlayByFile()
{
    if ( m_hReadThread != NULL )
    {
        return TRUE;
    }
    m_bReadThreadFinish = FALSE;
    VDSleep( 100 );

    memset(m_achFile,0,MAX_PATH);
    /*CGateTime cNow;*/
    s8 achTime[20] = {0};
    time_t now = time(NULL);
    struct tm *timenow;
    timenow = localtime(&now);
    strftime(achTime,100,"%Y%m%d%H%M%S",timenow);

    sprintf(m_achFile,TEMP_FILE_NAME, m_nPort, achTime);

    m_pFileWrite = fopen(m_achFile,"ab+");
    if (m_pFileWrite == NULL)
    {
        UNILOGE("CDhProxy.StartPlayByFile open m_pFileWrite:%s error",m_achFile);
        return FALSE;
    }

    m_pFileRead = fopen(m_achFile,"rb");
    if (m_pFileRead == NULL)
    {
        UNILOGE("CDhProxy.StartPlayByFile open m_pFileRead:%s error",m_achFile);
        return FALSE;
    }
    m_hReadThread = OspTaskCreate(Read2DecTask,"Read2DecTask",80,5*1024*1024,(KD_PTR)this,0,NULL);
    return TRUE;
}

//读取文件输入给大华解码库解码
void* CALLBACK CGat669Proxy::Read2DecTask( void* pParam )
{
    if ( NULL == pParam )
    {
        return 0;
    ...
    CGat669Proxy *pDec = (CGat669Proxy*)pParam;
    int nHdrSize = 0; 
    //TCHAR *szData = new TCHAR[DATA_IN_BYTE];
    u16* szData = new u16[DATA_IN_BYTE];
    while( TRUE )
    {
        if ( pDec->m_bReadThreadFinish )
        {
            break;
        }
        memset( szData, 0, DATA_IN_BYTE );
        s32 nSizeReadLen = fread(&nHdrSize, 1, sizeof(int), pDec->m_pFileRead);
        if ( nSizeReadLen < sizeof(int) )
        {
            fseek(pDec->m_pFileRead, -nSizeReadLen,  SEEK_CUR);
            Sleep( 100 );
            continue;
        }
        if ( nHdrSize > DATA_IN_BYTE)
        {
            UNILOGI("gat669[%d]<%s> size:0x%x > 0x%x", pDec->m_nPort, __FUNCTION__, nHdrSize, DATA_IN_BYTE);
            continue;
        }
        s32 nDataLen = nHdrSize;
        s32 nDataReadLen = fread( szData, 1, nDataLen, pDec->m_pFileRead);
        if ( nDataReadLen < nDataLen )
        {
            fseek(pDec->m_pFileRead, -(nDataReadLen+nSizeReadLen),  SEEK_CUR);
            continue;
        }

        int nReInput = 0;
        while( !pDec->m_cGat669.GAT669_InputVideoData( pDec->m_nPort, (BYTE*)szData, nDataLen ) && nReInput < 12 )
        {
            nReInput++;
            Sleep( 10*nReInput );
            UNILOGE("gat669[%d]<%s>buf:%u size:%u error:%s reput:%d",
                pDec->m_nPort, __FUNCTION__, szData, nDataLen, pDec->m_cGat669.GAT669_GetLastError(), nReInput);
        }
    }

    delete []szData;

    if (pDec->m_pFileRead != NULL)
    {
        fclose(pDec->m_pFileRead);
        pDec->m_pFileRead  = NULL;
    }
    return 0;
}
