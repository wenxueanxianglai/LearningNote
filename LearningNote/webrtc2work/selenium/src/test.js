...
class router4Log {
  ...
  async collect() {
    this._report.meetingInfo
      .labels(
        this._workerId,
        this.id,
        this._gateWayId,
        this._router.appData.meetingId
      )
      .set(1);
  }
}
...
class userInfoModel {
  constructor() {
    this.userInfo = new client.Gauge({
...
const webRtcTransportcfg = {
  ...
  info: {
    ...
    help: "user CPU time used",
    labelNames: ["work_id", "router_id", "transport_id", "transport_type"],
  },
  bytesReceived: {
...
class plainTransportModel {
  ...
  collect(plainData, workId, routerId, gateWayId, meetingId, userId, type) {
    plainData.forEach((item) => {
      //console.log("plainTransport collect begin~~~");
      this.info.labels(workId, routerId, item.transportId, item.type).set(1);
      ...
      //console.log("plainTransport collect end~~~: ", item.transportId);
    });
  }
}
...
class transport4Log {
  constructor(transport, workerId, routerId, gateWayId, meetingId) {
    ...
    this._userReport = userInfoModel.getInst();
  ...
  async collect() {
    ...
    if (statData.length > 0) {
      await this._userReport.collect(
        this._workerId,
        this._routerId,
        this._transport.id,
        this._gateWayId,
        this._meetingId,
        this._userId,
        this._transport.appData.userType,
        this._transport.constructor.name,
        this._transport.constructor.name == "WebRtcTransport"
          ? this._transport.appData.type
          : ""
      );
    }

    await this._report.collect(
      statData,
      this._workerId,
      this._routerId,
      this._gateWayId,
...
// interflow_userInfo
class interflowInfoModel {
  constructor() {
    this.userInfo = new client.Gauge({
      name: "btg_gateway_interflowinfo",
      help: "other gateway user--> native user",
      labelNames: [
        "gateway_id",
        "meeting_id",
        "user_id",
  ...
  static getInst() {
    if (!this.instance) {
      this.instance = new interflowInfoModel();
    }
    return this.instance;
  }
}
...
class producer4Log {
  constructor(producer, worker, router, transport) {
    this.id = producer.id;
    this._producer = producer;

    this._workerId = worker.pid;
    this._routerId = router.id;
    this._transportId = transport.id;
    this._transportType = transport.constructor.name;

    this._gateWayId = worker.appData.gateWayId;
    this._meetingId = router.appData.meetingId;
    this._userId = transport.appData.userId;
    this._userType = transport.appData.userType;
    this._interflowId = producer.appData.userId;

    this._report = producerModel.getInst();
    this._interflowReport = interflowInfoModel.getInst();
  }
  ...
  async collect() {
    ...
    if (
      statData.length > 0
      // && this._userType == "nativeUser"
      // && this._transportType == "PlainTransport"
    ) {
      this._interflowReport.collect(
        this._gateWayId,
        this._meetingId,
        this._userId,
        this._interflowId,
...
// subcribe_userInfo
class subcribeInfoModel {
  constructor() {
    this.subcribeInfo = new client.Gauge({
      name: "btg_gateway_subscribeinfo",
      help: "webuser-->subcribe",
      labelNames: [
        "gateway_id",
        "meeting_id",
        "user_id",
  ...
  static getInst() {
    if (!this.instance) {
      this.instance = new subcribeInfoModel();
    }
    return this.instance;
  }
}
...
class consumerModel {
  ...
  collect(consumerData, workId, routerId, transportId, consumerId) {
...
class consumer4Log {
  constructor(consumer, worker, router, transport) {
    this.id = consumer.id;
    ...
    this._userId = transport.appData.userId;
    this._userType = transport.appData.userType;
    this._subscribeId = consumer.appData.subscribeId;

    this._report = consumerModel.getInst();
    this._subscribeReport = subcribeInfoModel.getInst();
  }
  ...
  async collect() {
    ...
    if (statData.length > 0
      //&& this._transportType == "WebRtcTransport"
    ) {
      this._subscribeReport.collect(
        this._gateWayId,
        this._meetingId,
        this._userId,
        this._subscribeId,
...
class Monitor {
  ...
  createTransportLog(transport, workerId, routerId, gateWayId, meetingId) {
    console.log("createTransportLog: ", transport.constructor.name);
    let transport_ = new transport4Log(
      transport,
      workerId,
      routerId,
      gateWayId,
  ...
  createProducerLog(producer, worker, router, transport) {
    let producer_ = new producer4Log(producer, worker, router, transport);
  ...
  createConsumerLog(consumer, worker, router, transport) {
    let consumer_ = new consumer4Log(consumer, worker, router, transport);
    ...
    mediasoup.on("newworker", (worker) => {
      ...
      worker.observer.on("newrouter", (router) => {
        ...
        router.observer.on("newtransport", (transport) => {
          this.createTransportLog(
            transport,
            worker.pid,
            router.id,
            worker.appData.gateWayId,
            router.appData.meetingId
          );
          ...
          transport.observer.on("newproducer", (producer) => {
            this.createProducerLog(producer, worker, router, transport);
          ...
          transport.observer.on("newconsumer", (consumer) => {
            ...
            this.createConsumerLog(consumer, worker, router, transport);
