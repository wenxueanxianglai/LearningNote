...
## 修改容器内的元素
### 6.11.1 容器元素的必要条件
*  元素必须可复制或可搬移(copyable or moveable)
  * 元素类型必须隐式或显示提供一个 **copy** 或 **move** 构造函数
  * 被制造出来的拷贝应该与原件等效，相等性测试结果相等，且 原件与拷贝的 Operator相同
* 元素必须可被 assignment 操作符加以搬移或赋值。
  * 容器和算法以新元素复写旧元素时用的是assignment
* 元素必须可被一个析构函数销毁
  * 当元素被移除(remove),容器会销毁该元素的内部拷贝
  * 析构函数一定不能是private
  * 析构函数一定不可抛出异常


* 对序列式容器而言，元素的default构造函数必须可用
  * 我们可以在没有给予任何初值的情况下创建一个非空容器，或增加容器的元素个数，这些元素都将以default构造函数完成
  * 对于某些操作必须定义 == 如果你想查找元素，这一点特别重要
  * 在关联式容器中，元素必须定义适用于排序准则的操作， < .
  * 无序容器应该为元素提供一个hash函数 和一个等同测试准则
  

### 6.11.2 value语义 vs Reference语义
* 通常，所有元素都会建立 **元素拷贝**， 返回也是 **元素拷贝**
* 意味着容器内的元素与自己放进去的相等但 **非同一**
* 修改容器内的元素，实际上改变的是拷贝，而不是原件
